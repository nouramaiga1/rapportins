...
data_filtree = data[data["commune"] == "SAN-PEDRO"]
...
color_map = {genre: 'blue' if genre == 'Masculin' else 'red' for genre in category_counts.index}
...
st.markdown("<h2>Top 2 activité : Restaurants, maquis, caves et buvettes</h2>", unsafe_allow_html=True)

restaurants_data = data[data["activity_sector"] == "RESTAURANT/MAQUIS/CAVE/BUVETTE"]

number_of_rows_restaurants = len(restaurants_data)

total_ca_restaurants = restaurants_data['CA'].sum()

average_ca_restaurants = restaurants_data['CA'].mean()

average_age_restaurants = restaurants_data['ages'].mean()

formatted_number_vetements = f"{number_of_rows_restaurants:,.0f}".replace(",", " ")

category_counts = restaurants_data["genre"].value_counts()
...
category_counts = restaurants_data["tranche_age"].value_counts()
...
restaurants_data = data[data["activity_sector"] == "RESTAURANT/MAQUIS/CAVE/BUVETTE"]
category_counts = restaurants_data["stade_de_maturite"].value_counts()
fig71 = px.pie(
    names=category_counts.index,  # Catégories uniques
    values=category_counts.values,  # Quantités associées à chaque catégorie
    hole=0.3,  # Pour un donut chart (optionnel)
...
with col1:
    ...
    st.metric(label="Chiffre d'Affaires Global", value=f"{total_ca_restaurants:,.0f} FCFA")
    st.write("")
    st.write("")
    st.write("")
    st.write("")
    st.write("")
    st.write("")
    st.write("La restauration est pratiquée en majorité par des femmes.")
    st.write("La plupart des restaurateurs sont jeunes, c'est-à-dire ont moins de 40 ans.")
    st.write("Les activités de ce secteur ont tendance à être matures.")
    st.write("")
    st.write("")
    st.write("")
    st.write("")
...
with col2:
    st.metric(label="Moyenne d'Âge", value=f"{average_age_restaurants:,.1f} ans")
    st.metric(label="Moyenne du Chiffre d'Affaires", value=f"{average_ca_restaurants:,.0f} FCFA")
...
st.markdown("<h2>Top 3 activité : Coiffure, cosmétique et esthétique</h2>", unsafe_allow_html=True)




produits_agricoles_data = data[data["activity_sector"] == "COIFFURE/COSMETIQUE/ESTHETIQUE"]
...
with col2:
    ...
    st.write("")
    st.write("")
    st.write("Le domaine de la beauté est dominé par les hommes.")
    st.write("Les entreprenants dans ce domaine sont des jeunes.")
    st.write("Les activités du secteur sont en majorité matures.")
    st.write("")
    st.write("")
    st.write("")
    st.write("")
